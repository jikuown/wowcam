"use client";

import { motion } from "framer-motion";
import { TESTIMONIALS } from "@/lib/constants";

const INITIALS_BG = ["#1A1A1A", "#2D1A1A", "#1A2D1A"];

export default function Testimonials() {
  return (
    <section id="testimonials" className="py-28 px-6 bg-white">
      <div className="max-w-6xl mx-auto">

        <motion.div
          className="mb-14"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
        >
          <p className="t-caption mb-4">Reviews</p>
          <h2 className="t-headline text-black">Para pengguna<br />sudah buktikan.</h2>
        </motion.div>

        {/* Mobile: horizontal scroll */}
        <div className="md:hidden overflow-x-auto scrollbar-hidden pb-4">
          <div className="flex gap-4 w-max">
            {TESTIMONIALS.map((t, i) => (
              <div
                key={t.name}
                className="w-[300px] shrink-0 rounded-3xl p-7 bg-[#F5F5F7] flex flex-col gap-5"
              >
                <div className="flex gap-0.5">
                  {Array.from({ length: t.rating }).map((_, j) => (
                    <span key={j} className="text-[#FF00BD] text-sm">★</span>
                  ))}
                </div>
                <blockquote className="text-[15px] leading-relaxed text-black flex-1">
                  &ldquo;{t.quote}&rdquo;
                </blockquote>
                <div className="flex items-center gap-3 pt-4 border-t border-[#E8E8E8]">
                  {/* TODO: replace placeholder — real avatar */}
                  <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-[12px] font-bold shrink-0"
                    style={{ background: INITIALS_BG[i] }}>
                    {t.name[0]}
                  </div>
                  <div>
                    <p className="text-[13px] font-semibold text-black">{t.name}</p>
                    <p className="text-[12px]" style={{ color: "#86868B" }}>{t.title}</p>
                  </div>
                </div>
              </div>
            ))}
          </div>
        </div>

        {/* Desktop grid */}
        <div className="hidden md:grid grid-cols-3 gap-5">
          {TESTIMONIALS.map((t, i) => (
            <motion.div
              key={t.name}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.5, delay: i * 0.1 }}
              whileHover={{ y: -4 }}
              className="rounded-3xl p-8 bg-[#F5F5F7] flex flex-col gap-5 cursor-default"
            >
              <div className="flex gap-0.5">
                {Array.from({ length: t.rating }).map((_, j) => (
                  <span key={j} style={{ color: "#FF00BD" }} className="text-sm">★</span>
                ))}
              </div>
              <blockquote className="text-[15px] leading-relaxed text-black flex-1">
                &ldquo;{t.quote}&rdquo;
              </blockquote>
              <div className="flex items-center gap-3 pt-4 border-t border-[#E8E8E8]">
                {/* TODO: replace placeholder — real avatar */}
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-white text-[13px] font-bold shrink-0"
                  style={{ background: INITIALS_BG[i] }}>
                  {t.name[0]}
                </div>
                <div>
                  <p className="text-[14px] font-semibold text-black">{t.name}</p>
                  <p className="text-[12px]" style={{ color: "#86868B" }}>{t.title}</p>
                </div>
              </div>
            </motion.div>
          ))}
        </div>
      </div>
    </section>
  );
}
